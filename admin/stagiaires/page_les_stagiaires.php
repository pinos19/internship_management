	...
	if ($index_classe == 1)  $and_classe = " AND niveau like 'ING1%'";
	if ($index_classe == 2)  $and_classe = " AND niveau like 'ING2%'";


	$and_annee_scolaire = " AND sc.annee_scolaire='$annee_scolaire'";

	$requete_stagiaire = "SELECT 
				st.id, st.nom as nom_stagiaire,st.prenom,st.adr_mail_acad,st.entreprise_accueil as accueil,
				st.type_de_stage as typeStage, st.tuteur_interne as ti, st.tuteur_externe as te,
